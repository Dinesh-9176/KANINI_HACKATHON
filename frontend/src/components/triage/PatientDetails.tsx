...
// Fallback contributing factors for mock patients
const getDefaultContributingFactors = (patient: Patient): ContributingFactor[] => {
...
export function PatientDetails({ patient }: PatientDetailsProps) {
    ...
    const factors = patient.contributingFactors || getDefaultContributingFactors(patient)
    const RiskIcon = riskStyles.icon

    // Use real vitals from API or fallback defaults
    const vitals = patient.vitals
        ? [
            { label: "Blood Pressure", value: patient.vitals.bloodPressure, unit: "mmHg", icon: Activity },
            { label: "Heart Rate", value: patient.vitals.heartRate, unit: "bpm", icon: Heart },
            { label: "Temperature", value: patient.vitals.temperature, unit: "°F", icon: Thermometer },
            { label: "SpO2", value: patient.vitals.oxygenSaturation, unit: "%", icon: Wind },
        ]
        : [
        ...
        <div className="h-full overflow-y-auto p-6 space-y-8 custom-scrollbar">
            ...
            <div className="p-4 rounded-xl border bg-card text-card-foreground shadow-sm">
                ...
                <p className="text-sm text-muted-foreground">
                    {patient.predictedDisease
                        ? `Predicted condition: ${patient.predictedDisease.charAt(0).toUpperCase() + patient.predictedDisease.slice(1)}`
